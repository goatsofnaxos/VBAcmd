...
class VBAFSMthread(Thread):
    ...
    def __init__(self,window):
        self.window = window
        self.states = ['waiting4start', 'pullingBack', 'slackening', 'launching','jiggling']
        self.transitions = [
            ...
            {'trigger': 'jiggleorlaunch', 'source': 'launching',      'dest': 'jiggling',       'after': 'do_jiggling',        'conditions': 'animal_jiggled'},
            {'trigger': 'jiggleorlaunch', 'source': 'jiggling',       'dest': 'launching',      'after': 'do_launching',       'conditions': 'ok_to_launch'},
            {'trigger': 'jiggleorlaunch', 'source': 'jiggling',       'dest': 'pullingBack',    'after': 'do_pullingBack',     'conditions': 'animal_escaped'},
        ]
    ...
    def do_slackening(self):
        print self.state
        # Instruct servo to move to slack position (on ao0)
        # Check that position (on ai0) meets conditions (i.e. animal has not escaped or jiggled), send FSM to pullingBack if condition not met
        # When position (on ai0) has met conditions (i.e. animal has not escaped or jiggled) for > N seconds (optional: N variable), launch
    ...
    def do_jiggling(self):
        print self.state
        # Set trigger (do0) to low
        # Check that position (on ai0) meets conditions (i.e. animal has not escaped or jiggled), send FSM to pullingBack if animal escapes
        # When position (on ai0) has met conditions (i.e. animal has not escaped or jiggled) for > N seconds (optional: N variable), launch
        while True and (self.state is 'jiggling'):
            self.jiggleorlaunch()
    ...
    def animal_jiggled(self):
        bufLen = 0.5
        statevar = random.random_sample() > 0.8
        print '## ANIMAL JIGGLED? ##', statevar
        sleep(bufLen/2)
        return statevar
